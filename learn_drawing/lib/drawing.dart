...
class Drawing{
  ...
  Drawing._fromJson(Map<String,dynamic> json):_word=json['word'],
  _strokes=(json['drawing'] as Iterable).map((e) => Stroke.fromIterable(e[0],e[1])).toList();
  Drawing._fromJsonString(String jsonString):this._fromJson(jsonDecode(jsonString));
...
class Stroke{
  ...
  Stroke.fromIterable(Iterable xCords,Iterable yCords)
  :this._fromCordinatesList(xCords.map((e) =>e as int).toList(),yCords=yCords.map((e) =>e as int).toList());
  Stroke._fromCordinatesList(List<int> xCords,List<int> yCords){
    assert(xCords.length==yCords.length);
    List<Point<double>> pts=new List<Point<double>>();
    for(int i=0;i<xCords.length;i++){
      pts.add(Point(xCords[i].toDouble(), yCords[i].toDouble()));
    }
    this._points=pts;
  }
}